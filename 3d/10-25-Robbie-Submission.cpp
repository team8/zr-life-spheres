...
void init(){
    DEBUG(("### PALY ROBOTICS SUBMISSION 10/21 ###"));
...
//  zero[2] += .10;
    ...
    memcpy(fastDrillTarget, drillTarget, sizeof(float)*3);
    // fastDrillTarget[2] = .7;
...
void loop(){
    ...
    if (state == DRILLING) {
        ...
        if (game.getNumSamplesHeld() == 4 || geyserOnMe || (game.getNumSamplesHeld() > 0 && api.getTime() > 155 && game.getFuelRemaining() > .18)) {
            ...
            mathVecScale(geyserEvationTarget, .45/mathVecMagnitude(geyserEvationTarget,3));
