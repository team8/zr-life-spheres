        ...
        for (int i = 0; i < tiles; i++) {
            ...
            if (currentTile[0] >= -8 && currentTile[0] <= 8 &&
                ...
                && game.getDrills(currentTile) == 0) {
                    ...
                    // Also check to make sure that it is not in the opponent quadrant
                    if (game.getTerrainHeight(currentTile) < .41 && ((colorAdjust * currentTile[0]) > 0 || (colorAdjust * currentTile[1]) > 0)) {
                        DEBUG(("%d,%d,%d", currentTile[0], currentTile[1], currentTile[2]));
                        float newPos[3];
                        game.square2pos(currentTile, newPos);
                        currentTarget[0] = newPos[0];
                        currentTarget[1] = newPos[1];
                        currentTarget[2] = .40 - .10 - .030;  
                        return;
                    }
                }
...
// LOOP Function
void loop(){
    DEBUG(("PALY ROBOTICS SUBMISSION 11/12/17"));
    api.getMyZRState(myState);
    ...
    if (state == MOVING_TO_DRILL) {
        ...
        if (distance < .05 && velocity < .008 && fabsf(myState[2] - currentTarget[2]) < .02 && myState[2] < BOTTOM_WALL) {
    ...
    if (state == STARTING_DRILL) {
        ...
        if (rotV > .025 || fabsf(myState[8]) > .075) {
            ...
            if (api.getTime() > 160) drillTarget[2] = .9; // If this is the end of the game, go faster
             ...
             currentTarget[2] -= .025; // move up the current target slightly to stop drifting
        }
    }
